...
export function ProjectSidebar({
  ...
  return <div className={cn("relative z-20 flex flex-col h-full bg-sidebar border-r transition-all duration-300 shrink-0 overflow-hidden", isCollapsed ? "w-16" : "w-64")}>
      ...
      <ScrollArea className="flex-1" onScrollCapture={handleScroll}>
        <div className={cn("space-y-0.5", isCollapsed ? "p-1" : "p-3 pr-3") } ref={scrollRef}>
          {/* Folders */}
          ...
          return <div key={folder.id} className="space-y-0.5">
                ...
                {isExpanded && <div className="ml-8 space-y-0.5">
                    {folderProjects.map(project => <div key={project.id} className="relative group flex items-center w-full rounded-lg hover:bg-accent/50 transition-colors mx-0 my-0.5 overflow-hidden">
                        <Button variant="ghost" size="sm" className={cn("w-full max-w-full min-w-0 justify-start px-4 h-10 hover:bg-transparent text-left pr-12 overflow-hidden", currentProjectId === project.id && "bg-muted hover:bg-muted")} onClick={() => onProjectSelect(project.id)}>
                          <span className="text-sm truncate flex-1 min-w-0" title={project.title}>{project.title}</span>
                        </Button>
          ...
          {standaloneProjects.map(project => <div key={project.id} className="relative group flex items-center w-full rounded-lg hover:bg-accent/50 transition-colors mx-0 my-0.5 overflow-hidden">
              {isCollapsed ? (
                ...
                <>
                  <Button variant="ghost" size="sm" className={cn("w-full max-w-full min-w-0 justify-start rounded-lg h-10 text-left px-3 pr-12 hover:bg-transparent overflow-hidden", currentProjectId === project.id && "bg-muted hover:bg-muted")} onClick={() => onProjectSelect(project.id)}>
